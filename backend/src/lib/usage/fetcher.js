...
// ── Inline platform helpers (from appConstants) ───────────────────────────────
const PLATFORM = { DARWIN_AMD64: 1, DARWIN_ARM64: 2, LINUX_AMD64: 3, LINUX_ARM64: 4, WINDOWS_AMD64: 5 };
const IDE_TYPE = { ANTIGRAVITY: 9 };
const PLUGIN_TYPE = { GEMINI: 2 };
function getPlatformEnum() {
  const os = platform(); const a = arch();
  if (os === "darwin") return a === "arm64" ? PLATFORM.DARWIN_ARM64 : PLATFORM.DARWIN_AMD64;
  if (os === "linux")  return a === "arm64" ? PLATFORM.LINUX_ARM64  : PLATFORM.LINUX_AMD64;
  if (os === "win32")  return PLATFORM.WINDOWS_AMD64;
  return 0;
}
function getPlatformUserAgent() { return `antigravity/1.104.0 ${platform()}/${arch()}`; }
const CLIENT_METADATA = { ideType: IDE_TYPE.ANTIGRAVITY, platform: getPlatformEnum(), pluginType: PLUGIN_TYPE.GEMINI };

// ── fetch wrapper with timeout ────────────────────────────────────────────────
async function proxyAwareFetch(url, options = {}, _proxyOptions = null, timeoutMs = 10_000) {
